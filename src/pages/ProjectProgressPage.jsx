...
import { Lock, FileText, Upload, Info, Check, ChevronDown, ChevronRight, Layers, Target, Package } from 'lucide-react';
...
const ProjectProgressPage = () => {
  ...
  // Estado para manejar qué nodos están expandidos
  ...
  // Función para saber si un nodo está expandido (por defecto true)
  const isExpanded = (nodeId) => {
    return expandedNodes[nodeId] !== false;
  };

  // Estado para los KPIs y datos del proyecto (inicializado con la estructura esperada)
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        // Asumiendo que la API devuelve los datos dentro de response.data
        ...
        setProgressData(prevData => ({
          ...
          // Si el backend trae las fases/hitos, los usamos, sino usamos un mock por ahora
  ...
  // Función auxiliar para tener datos mockeados en la tabla temporalmente
  const getMockFases = () => [
    {
        id: 'F1',
    ...
    <div className="progress-page-container">
      ...
      <header className="progress-header">
        <div className="header-title">
          ...
          <span className="project-id-badge">— {progressData.codigo}</span>
        </div>
      </header>
                    ...
                    <React.Fragment key={hito.id}>
                      ...
                      {isExpanded(`hito-${hito.id}`) && hito.entregables.map((ent) => (
                        <tr key={ent.id} className="row-deliverable">
                          ...
                          {/* Columna extra implícita para botón de acción flotante */}
                          {ent.estado !== 'A conformidad' && (
